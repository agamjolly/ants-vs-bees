...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        """
        distance,location=0,self.place
        while location != beehive: 
            if location.bees and self.min_range<=distance and distance<=self.max_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        more_bees=self.place.bees[:]
        if more_bees != None:
            for bee in more_bees: 
...
# END Problem 13
    ...
    def remove_from(self, place):
        return None if not self.is_fake and isinstance(self,QueenAnt) else Ant.remove_from(self,place)
    # END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.impostor: 
            ...
            self.is_fake=False
        # lst = []
        self.lst,QueenAnt.impostor=[],True
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_fake:
            ...
            if self.nearest_bee(gamestate.beehive):
                self.nearest_bee(gamestate.beehive).reduce_armor(self.damage)
            old_state = self.place.exit
            while old_state:
                if old_state.ant:
                    if old_state.ant not in self.lst:
                        old_state.ant.damage*=2
                        self.lst.append(old_state.ant)
                    if isinstance(old_state.ant, ContainerAnt) and old_state.ant.can_contain and old_state.ant.contained_ant not in self.lst and old_state.ant.contained_ant:
                        old_state.ant.contained_ant.damage*=2
                        self.lst.append(old_state.ant.contained_ant)

                old_state = old_state.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        Ant.reduce_armor(self,amount=self.armor)
        if self.armor<=0 and not self.is_fake: 
            bees_win()
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.place.entrance and not self.future: 
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    i,new_action=0,bee.action
    def check(gamestate): 
        nonlocal i
        i+=1
        if i<=duration: 
            return effect(new_action,bee)(gamestate)
